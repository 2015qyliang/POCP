...
for (gn in genome.files) {
  ...
  commond.makedb <- paste0('usearch.exe -makeudb_ublast Rawdata/',
                           gn, ' -output Database/', header.file)
  system(commond.makedb)
}

# pair blast
genome.comn <- combn(genome.files,2)
blast.comm1 <- 'usearch.exe -ublast Rawdata/'
blast.comm2 <- ' -db Database/'
blast.comm3 <- ' -evalue 1e-5 -accel 0.5 -id 0.4 -blast6out Result/'
pocp.vector <- c()
for (i in (1:dim(genome.comn)[2]) ) {
  ...
  system(paste0("python changeUsearchOutFormat.py ",result.forward))
  ...
  system(paste0("python changeUsearchOutFormat.py ",result.backward))
